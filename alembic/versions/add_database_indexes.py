...
def upgrade() -> None:
    ...
    op.create_index('ix_bookings_booking_address_id', 'bookings', ['booking_address_id'])
    ...
    op.create_index('ix_services_is_qualified', 'services', ['is_qualified'])
    ...
    # Nurse Documents
    op.create_index('ix_nurse_documents_verification_status', 'nurse_documents', ['verification_status'])
    
    # ========== DATE/TIME INDEXES ==========
    # These improve range queries and sorting
    
    # Bookings
    op.create_index('ix_bookings_scheduled_start_time', 'bookings', ['scheduled_start_time'])
    op.create_index('ix_bookings_scheduled_end_time', 'bookings', ['scheduled_end_time'])
    ...
    op.create_index('ix_availability_nurse_booked', 'availability', ['nurse_id', 'is_booked'])
    
    # Find nurse's upcoming bookings by status
    op.create_index('ix_bookings_nurse_status', 'bookings', ['nurse_id', 'booking_status'])
    op.create_index('ix_bookings_patient_status', 'bookings', ['patient_id', 'booking_status'])
    
    # Find bookings in time range by status
    op.create_index('ix_bookings_status_time', 'bookings', ['booking_status', 'scheduled_start_time'])
    ...
    op.create_index('ix_availability_nurse_time', 'availability', ['nurse_id', 'start_time', 'end_time'])
    
    # Location-based queries (if you implement geo-search)
    op.create_index('ix_addresses_city_state', 'addresses', ['city', 'state'])
    
    # Payment tracking
    op.create_index('ix_payments_patient_status', 'payments', ['patient_id', 'payment_status'])
...
def downgrade() -> None:
    ...
    # Filter column indexes
    op.drop_index('ix_nurse_documents_verification_status', table_name='nurse_documents')
    ...
    op.drop_index('ix_services_is_qualified', table_name='services')
