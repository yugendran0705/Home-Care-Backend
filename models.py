...
class User(Base):
    ...
    user_type = Column(
        String(20), nullable=False, comment="ENUM: Patient, Nurse, Admin"
    )
    ...
    patient = relationship(
        ...
        back_populates="user",
        uselist=False,
        primaryjoin="User.id == Patient.id",
    )
    nurse = relationship(
        "Nurse", back_populates="user", uselist=False, primaryjoin="User.id == Nurse.id"
    )
...
class Patient(Base):
    ...
    id = Column(
        "patient_id", UUID(as_uuid=True), ForeignKey("users.user_id"), primary_key=True
    )
    ...
    gender = Column(
        String(10), nullable=True, comment="ENUM: Male, Female, Other"
    )  # Nullable
    address_id = Column(
        UUID(as_uuid=True), ForeignKey("addresses.address_id"), nullable=True
    )  # Nullable, as per DBML relationship definition

    # Relationships
    user = relationship("User", back_populates="patient", uselist=False)
    primary_address = relationship(
        "Address", primaryjoin="Patient.address_id == Address.id"
    )  # For patient's primary address
...
class Nurse(Base):
    ...
    id = Column(
        "nurse_id", UUID(as_uuid=True), ForeignKey("users.user_id"), primary_key=True
    )
    first_name = Column(String(100), nullable=False)
    last_name = Column(String(100), nullable=False)
    phone_number = Column(String(20), unique=True, nullable=False)
    ...
    primary_address = relationship(
        "Address", primaryjoin="Nurse.address_id == Address.id"
    )  # For nurse's primary address
...
class Service(Base):
    ...
    )  # DECIMAL(10,2) mapped to Numeric
    duration = Column(Integer, nullable=True)  # Nullable
    duration_type = Column(
        String(20), nullable=True, comment="ENUM: Minutes, Hours, Days"
    )  # Nullable
    is_active = Column(Boolean, nullable=False, default=True)
...
class NurseService(Base):
    ...
    nurse_id = Column(
        UUID(as_uuid=True), ForeignKey("nurses.nurse_id"), primary_key=True
    )
    service_id = Column(
        UUID(as_uuid=True), ForeignKey("services.service_id"), primary_key=True
    )
...
class Availability(Base):
    ...
    id = Column(
        "availability_id", UUID(as_uuid=True), primary_key=True, default=uuid.uuid4
    )
...
class Booking(Base):
    ...
    booking_status = Column(
        ...
        nullable=False,
        default="Pending",
        comment="ENUM: Pending, Confirmed, Completed, Cancelled, Rejected",
    )
    total_amount = Column(
    ...
    payment_status = Column(
        ...
        nullable=False,
        default="Pending",
        comment="ENUM: Pending, Paid, Refunded, Failed",
    )
    booking_address_id = Column(
        UUID(as_uuid=True), ForeignKey("addresses.address_id"), nullable=False
    )
    ...
    booking_address = relationship(
        "Address", primaryjoin="Booking.booking_address_id == Address.id"
    )
    review = relationship(
...
class Payment(Base):
    ...
    payment_status = Column(
        ...
        nullable=False,
        default="Initiated",
        comment="ENUM: Initiated, Success, Failed, Refunded",
    )
...
class NurseDocument(Base):
    ...
    document_type = Column(
        String(50), nullable=False, comment="ENUM: Aadhar, PAN, NursingLicense, Other"
    )
    document_url = Column(
        String(255),
        nullable=False,
        comment="URL to the stored document (e.g., in an S3 bucket)",
    )
    verification_status = Column(
        String(20),
        nullable=False,
        default="Pending",
        comment="ENUM: Pending, Approved, Rejected",
    )
    notes = Column(
        Text,
        nullable=True,
        comment="Optional notes from an admin regarding verification",
    )
