...
class BlackoutDateService:
  ...
  def create_blackout_date(
    ...
    if not blackout_data.start_datetime:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Start datetime is required."
        )

    if not blackout_data.end_datetime:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="End datetime is required."
        )
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to create blackout date: {str(e)}"
        )
  ...
  def get_blackout_date(
    ...
    blackout_date = self.blackout_repo.get_by_id(
  ...
  def get_nurse_blackout_dates(
    ...
    blackout_dates = self.blackout_repo.get_by_nurse_id(
        nurse_id=nurse_id
    )
  ...
  def update_blackout_date(
    ...
    start_datetime = update_data.get(
      "start_datetime",
      blackout_date.start_datetime
    )

    end_datetime = update_data.get(
      "end_datetime",
      blackout_date.end_datetime
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to update blackout date: {str(e)}"
        )
